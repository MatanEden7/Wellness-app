...
void main() {
  group('foods: exclusions', () {
    test('a food carrying an excluded allergen does not fit', () {
      final yogurt = _food('Greek Yogurt', {FoodTag.dairy, FoodTag.animalProduct});
      final result = ProfileFit.foodFit(yogurt, _profile(exclusions: ['dairy']));
    ...
    test('every onboarding exclusion id maps to a tag', () {
      ...
      for (final id in const ['dairy', 'gluten', 'nuts', 'eggs', 'shellfish', 'soy']) {
        expect(FoodTag.forExclusion(id), isNotNull, reason: '$id has no FoodTag');
      }
    });

    test('the "none" chip is not treated as an allergen', () {
      final chicken = _food('Chicken', {FoodTag.meat});
      expect(ProfileFit.foodFits(chicken, _profile(exclusions: ['none'])), isTrue);
    });
  });
  ...
  group('foods: diet type', () {
    test('herbivore rejects meat, fish and non-flesh animal products alike', () {
      ...
      expect(ProfileFit.foodFits(_food('Beef', {FoodTag.meat}), profile), isFalse);
      expect(ProfileFit.foodFits(_food('Salmon', {FoodTag.fish}), profile), isFalse);
      expect(
          ProfileFit.foodFits(
    ...
    test('herbivore accepts plant foods', () {
      ...
      expect(ProfileFit.foodFits(_food('Tofu', {FoodTag.soy}), profile), isTrue);
    });
  ...
  test('untagged food fits every profile', () {
    ...
    final profile = _profile(dietType: 'herbivore', exclusions: ['dairy', 'nuts']);
  ...
  group('exercises: equipment', () {
    test('bodyweight work fits even with no equipment', () {
      ...
      expect(ProfileFit.exerciseFits(pushups, _profile(equipment: ['none'])), isTrue);
    });

    test('barbell work does not fit someone with no equipment', () {
      final bench = _exercise('Bench Press', equipment: {Equipment.barbellRack});
      final result = ProfileFit.exerciseFit(bench, _profile(equipment: ['none']));
    ...
    test('an exercise fits if the user owns ANY of its listed equipment', () {
      ...
      final row = _exercise('Row', equipment: {Equipment.dumbbells, Equipment.cable});
      expect(ProfileFit.exerciseFits(row, _profile(equipment: ['cable'])), isTrue);
      expect(ProfileFit.exerciseFits(row, _profile(equipment: ['dumbbells'])), isTrue);
      expect(ProfileFit.exerciseFits(row, _profile(equipment: ['bands'])), isFalse);
    });
    ...
    test('injury outranks equipment: unsafe is reported even if owned', () {
      ...
      expect(
        ProfileFit.exerciseFit(
                ohp, _profile(equipment: ['none'], injuries: ['shoulder']))
            .failure,
        FitFailure.injury,
      );
    });
  ...
  group('templates', () {
    test('a meal template fails if any single item fails', () {
      ...
      final template = MealTemplate(
        ...
        updatedAt: DateTime(2026, 1, 1),
        items: const [
          MealTemplateItem(id: 'i1', templateId: 't', foodId: 'banana', amount: 1),
          MealTemplateItem(id: 'i2', templateId: 't', foodId: 'oats', amount: 1),
        ],
      );

      final result =
          ProfileFit.mealTemplateFit(template, foods, _profile(exclusions: ['gluten']));
      expect(result.fits, isFalse);
    ...
    test('a dangling foodId does not hide the whole template', () {
      final template = MealTemplate(
        ...
        updatedAt: DateTime(2026, 1, 1),
        items: const [
          MealTemplateItem(id: 'i1', templateId: 't', foodId: 'ghost', amount: 1),
        ],
      );

      expect(
          ProfileFit.mealTemplateFits(template, {}, _profile(exclusions: ['gluten'])),
          isTrue,
    ...
    test('a workout template fails if any exercise is contraindicated', () {
      ...
      const template = WorkoutTemplate(
        ...
        name: 'Upper',
        exercises: [
          TemplateExercise(id: 'e1', templateId: 'w', exerciseId: 'squat', orderIndex: 0),
          TemplateExercise(id: 'e2', templateId: 'w', exerciseId: 'ohp', orderIndex: 1),
        ],
      );
