...
> **Note**: This is the original design spec (Dec 2024) and describes intended
> architecture. For what's actually implemented today — including the in-memory
> storage reality and the food unit/nutrition math — see
> [`REPO_GUIDE.md`](REPO_GUIDE.md), which is kept up to date with the code.
...
```sql
-- Food catalog
-- `unit` (e.g. "100g", "g", "ml", "oz", "piece") determines how `amount` on
-- meal_items is interpreted — see FoodServingKind / FoodNutritionMath in
-- lib/features/meals/domain/. It is not a free label, it drives real math.
foods (id, name, unit, kcal_per_unit, protein_per_unit, carbs_per_unit, fat_per_unit)

-- User meals
meals (id, name, logged_at)
-- amount is the *normalized/stored* quantity (see FoodNutritionMath.storedQuantity),
-- not necessarily what the user typed. kcal/protein/carbs/fat are a denormalized
-- snapshot computed once at creation time via FoodNutritionMath.computeMacros —
-- they do not update if the underlying food's nutrition later changes.
meal_items (id, meal_id, food_id, amount, kcal, protein, carbs, fat)
...
```dart
// Nutrition
class FoodItem { id, name, unit, kcalPerUnit, proteinPerUnit, carbsPerUnit, fatPerUnit }
