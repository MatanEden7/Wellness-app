...
class DummyDataService {
  ...
  Future<void> _createMealTemplates(List<FoodItem> foods) async {
    ...
    await mealsRepository.createMealTemplate(breakfast.copyWith(items: breakfastItems));
  ...
  Future<void> _createMeals(List<FoodItem> foods) async {
    ...
    for (int dayOffset = 0; dayOffset < 14; dayOffset++) {
      ...
      await mealsRepository.createMeal(breakfast.copyWith(items: breakfastItems));
      ...
      if (dayOffset % 3 == 0) {
        ...
        await mealsRepository.createMeal(morningSnack.copyWith(items: snackItems));
      }
      ...
      if (dayOffset % 2 == 0) {
        ...
        await mealsRepository.createMeal(afternoonSnack.copyWith(items: snackItems));
      }
      ...
      if (dayOffset % 4 == 0) {
        ...
        await mealsRepository.createMeal(eveningSnack.copyWith(items: snackItems));
      }
    }
  }
  ...
  Future<List<WorkoutTemplate>> _createWorkoutTemplates(List<Exercise> exercises) async {
  ...
  Future<void> _createWorkoutSessions(
    ...
    for (int i = 0; i < workoutDays.length; i++) {
      ...
      final session = WorkoutSession(
        ...
        startedAt: startTime,
        endedAt: endTime,
        note: i % 3 == 0 ? 'Great workout!' : (i % 3 == 1 ? 'Solid session' : 'Felt strong today'),
        sets: [],
      );
  ...
  Future<List<TemplateExercise>> _getTemplateExercises(String templateId) async {
    final template = await workoutTemplatesRepository.getTemplateById(templateId);
  ...
  Future<void> _createSleepEntries() async {
    ...
    for (int dayOffset = 1; dayOffset <= 14; dayOffset++) {
      ...
      final entry = SleepEntry.create(
        ...
        endedAt: sleepEnd,
        quality: quality,
        note: quality >= 4 ? 'Slept well' : (quality == 3 ? 'Decent sleep' : 'Could be better'),
      );
  ...
  Future<void> _createRecurringScheduledEvents(List<WorkoutTemplate> templates) async {
    final now = DateTime.now();
    ...
    for (final mealTime in mealTimes) {
      // Start from tomorrow
      final tomorrow = DateTime(now.year, now.month, now.day + 1, mealTime['hour'] as int, mealTime['minute'] as int);
  ...
  Future<void> generateProfileData(String profileKey, bool useHebrew) async {
    ...
    switch (profileKey) {
      ...
      case 'women':
        ...
        break;
      case 'shoulder':
        exerciseDataList = profileService.getShoulderTherapyExercises(useHebrew);
        templateDataList = profileService.getShoulderTherapyTemplates(useHebrew);
        break;
      case 'back':
    ...
    await _createProfileWorkoutSessions(exercises, templateDataList, profileKey);
  ...
  Future<void> _createProfileWorkoutSessions(
    ...
    for (int i = 0; i < sessionDays.length && i < templateDataList.length; i++) {
      final daysAgo = sessionDays[i];
      final sessionStart = now.subtract(Duration(days: daysAgo, hours: 10));
      final sessionEnd = sessionStart.add(Duration(minutes: 45 + _random.nextInt(30)));
      ...
      for (final exerciseData in templateData.exercises) {
        ...
        for (int setNum = 0; setNum < exerciseData.defaultSets; setNum++) {
          final reps = exerciseData.defaultReps + _random.nextInt(3) - 1; // ±1 rep variation
          final weight = exerciseData.defaultWeight != null
              ? exerciseData.defaultWeight! + _random.nextDouble() * 5 // ±2.5kg variation
              : null;
